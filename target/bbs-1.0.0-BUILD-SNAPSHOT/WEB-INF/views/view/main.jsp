...
<body style="width: 1300px; margin-left: auto; margin-right: auto;">
	...
	<!---------아이템 뿌리기------------------------------------------------------------------------------------------------------------>
		...
		<c:if test="${pdto.startPage != 1 }">
			<input type="button" value="&#60&#60" onclick="location.href='/bbs/Item/selectall?curPage=${pdto.startPage - 1}&findKey=${pdto.findKey }&findValue=${pdto.findValue}'" style="font-size:16px; background-color: white; border: 0px;">
		</c:if>
		
		<c:forEach begin="${pdto.startPage }"  end="${pdto.endPage }" var="i">
			<c:if test="${i eq pdto.curPage}"><input type="button" value="${i }" onclick="location.href='/bbs/Item/selectall?curPage=${i}&findKey=${pdto.findKey }&findValue=${pdto.findValue}'" style="font-size:16px; width:35px; background-color: white; border: 0px; color: black;"></c:if>
			<c:if test="${i ne pdto.curPage}"><input type="button" value="${i }" onclick="location.href='/bbs/Item/selectall?curPage=${i}&findKey=${pdto.findKey }&findValue=${pdto.findValue}'" style="font-size:16px; width:35px; background-color: white; border: 0px; color: gray;" ></c:if>
		</c:forEach>
		
		<c:if test="${pdto.endPage != pdto.totPage }">
			<input type="button" value="&#62&#62" onclick="location.href='/bbs/Item/selectall?curPage=${pdto.endPage + 1}&findKey=${pdto.findKey }&findValue=${pdto.findValue}'" style="font-size:16px; background-color: white; border: 0px;">
		</c:if>
	</div>
	<br><br><br><br><br><br>
</body>
